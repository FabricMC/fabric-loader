...
public final class TomlReader {
	...
	public Map<String, TomlElement> read() {
		...
		while (hasNext()) {
			...
			Map<String, TomlElement> value = nextTableContent();
			
			// -- Saves the value --
			Map<String, TomlElement> valueMap = map;// the map that contains the value
			for (int i = 0; i < keyParts.size() - 1; i++) {
				String part = keyParts.get(i);
				Object child = valueMap.get(part);
				Map<String, TomlElement> childMap;
			...
			if (twoBrackets) {// element of a table array
				...
				if (tableArray == null) {
					...
					valueMap.put(name, new TomlElement(tableArray));
				}
				tableArray.add(value);
			} else {// just a table
				valueMap.put(keyParts.get(keyParts.size() - 1), new TomlElement(value));
			}
	...
	private Map<String, TomlElement> nextTableContent() {
		...
		while (true) {
			...
			map.put(name, new TomlElement(value));
		}
	}
